...
def calc_mixed_layer(var,psfc,p,dz,level=100.):
    ...
    var_100mb = np.ma.average(masked_var, axis=2, weights=dz)
    return var_100mb
...
def calc_t_dim(th, p):
    """
    Calculates temperature for a np.ndarray with dim=5
...
#######################
   base_t_bcst = np.expand_dims(base_t, 2)	


   base_th = calc_th(base_t_bcst, base_p)
   base_th_bcst = np.squeeze(base_th, 2)
   print('Theta-e:', th_e_base.shape)
   print('t_base', base_t.shape)
   print('p:', p.shape)
   ...
   p_diff = base_p - p
   print('Theta:', base_th.shape)
   masked_p_diff = np.ma.masked_where((p_diff) > 0, (p_diff))                   #mask indices above p_base
   base_index = np.abs(masked_p_diff).argmin(axis=2)                            #find indices of nearest values in p below p_base

   #make dry adiabatic profile:
   for k in range(1, p.shape[2]):
      print(k, calc_t(base_th, p[:,:,k,:,:]).shape)
      dry_parcel[:,:,k,:,:] = np.where(k > base_index, calc_t(base_th_bcst, p[:,:,k,:,:]), dry_parcel[:,:,k,:,:])

   #make moist adiabatic profile:
   for k in range(1, p.shape[2]):
      print(th_e_base.shape)
...
def calc_wind_shear(u, z, hgt, lower, upper):
...
def calc_bunkers(p, z, hgt, dz, u, v):
    ...
    shear_u = calc_wind_shear(u, z_agl, hgt, lower, upper)
    shear_v = calc_wind_shear(v, z_agl, hgt, lower, upper)
...
def calc_srh(z, u, v, hgt, dz, lower, upper, sm_u, sm_v):
