...
for i in range(len(dirs)):
   ...
   # print(files)
    ...
    for j in range(len(files)):
       ...
       # Calculate variables for sounding grid
        lat_snd = ds['XLAT'][0,::10,::10].values
        lon_snd = ds['XLONG'][0,::10,::10].values
        
        p_snd = p[:,::10,::10]
        t_snd = t[:,::10,::10] - 273.15
        td_snd = td[:,::10,::10] - 273.15
        u_snd = (u[:,::10,::10]*units.meter_per_second).to(units.knot).magnitude
        v_snd = (v[:,::10,::10]*units.meter_per_second).to(units.knot).magnitude
        z_snd = z[:,::10,::10] 
    ...
    # Interpolate desired variables to mandatory pressure levels
        ...
        ds = ds.drop(['PH', 'PHB', 'P', 'PB', 'T', 'QVAPOR', 'Q2', 'HGT', 
                      'U', 'V', 'W', 'TH2', 'PSFC', 'P_TOP', 'REFL_10CM'])
        ...
        if [s + '.nc' for s in files[:j]] == []:
            ...
            # Hourly swaths
        if [s + '.nc' for s in files[:j]][-12:] == []:
            uh_hrswt_02 = np.expand_dims(uh_02, 0)
            uh_hrswt_25 = np.expand_dims(uh_25, 0)
            uh_hrswt_wrf25 = ds['UP_HELI_MAX'].values
            wz_hrswt_02 = np.expand_dims(vort02, 0)
            wz_hrswt_25 = np.expand_dims(vort25, 0)
            dbz_hrswt = ds['REFD_MAX'].values
            
            #if not first file, open all previous files and take maximum value of all times
        else:
                # open all files already processed (these will contain '.nc' extension)
            ds_swt = xr.open_mfdataset([s + '.nc' for s in files[:j][-12:]], concat_dim='Time', 
                             combine='nested', compat='override', coords='all', 
                                drop_variables= ['UH_SWT02','UH_SWT25','UH_SWT25_WRF',
                                                'WZ_SWT02', 'WZ_SWT25', 'CREF_SWT'])
            ...
            ds_swt = xr.concat([ds[swt_vars], ds_swt[swt_vars]], dim='Time')
                # calculate swaths
            uh_hrswt_02 = np.expand_dims(np.amax(ds_swt['UH_02'].values, 0), 0)
            uh_hrswt_25 = np.expand_dims(np.amax(ds_swt['UH_25'].values, 0), 0)
            uh_hrswt_wrf25 = np.expand_dims(np.amax(ds_swt['UP_HELI_MAX'].values, 0), 0)
            wz_hrswt_02 = np.expand_dims(np.amax(ds_swt['WZ_02'].values, 0), 0)
            wz_hrswt_25 = np.expand_dims(np.amax(ds_swt['WZ_25'].values, 0), 0)
            dbz_hrswt = np.expand_dims(np.amax(ds_swt['REFD_MAX'].values, 0), 0)

             # 30-min swaths
        if [s + '.nc' for s in files[:j]][-6:] == []:
            uh_30swt_02 = np.expand_dims(uh_02, 0)
            uh_30swt_25 = np.expand_dims(uh_25, 0)
            uh_30swt_wrf25 = ds['UP_HELI_MAX'].values
            wz_30swt_02 = np.expand_dims(vort02, 0)
            wz_30swt_25 = np.expand_dims(vort25, 0)
            dbz_30swt = ds['REFD_MAX'].values
            
            #if not first file, open all previous files and take maximum value of all times
        else:
                # open all files already processed (these will contain '.nc' extension)
            ds_swt = xr.open_mfdataset([s + '.nc' for s in files[:j][-6:]], concat_dim='Time', 
                             combine='nested', compat='override', coords='all', 
                                drop_variables= ['UH_SWT02','UH_SWT25','UH_SWT25_WRF',
                                                'WZ_SWT02', 'WZ_SWT25', 'CREF_SWT'])
            ...
            ds_swt = xr.concat([ds[swt_vars], ds_swt[swt_vars]], dim='Time')
                # calculate swaths
            uh_30swt_02 = np.expand_dims(np.amax(ds_swt['UH_02'].values, 0), 0)
            uh_30swt_25 = np.expand_dims(np.amax(ds_swt['UH_25'].values, 0), 0)
            uh_30swt_wrf25 = np.expand_dims(np.amax(ds_swt['UP_HELI_MAX'].values, 0), 0)
            wz_30swt_02 = np.expand_dims(np.amax(ds_swt['WZ_02'].values, 0), 0)
            wz_30swt_25 = np.expand_dims(np.amax(ds_swt['WZ_25'].values, 0), 0)
            dbz_30swt = np.expand_dims(np.amax(ds_swt['REFD_MAX'].values, 0), 0)
        
            # 15-min swaths
        if [s + '.nc' for s in files[:j]][-3:] == []:
            uh_15swt_02 = np.expand_dims(uh_02, 0)
            uh_15swt_25 = np.expand_dims(uh_25, 0)
            uh_15swt_wrf25 = ds['UP_HELI_MAX'].values
            wz_15swt_02 = np.expand_dims(vort02, 0)
            wz_15swt_25 = np.expand_dims(vort25, 0)
            dbz_15swt = ds['REFD_MAX'].values
            
            #if not first file, open all previous files and take maximum value of all times
        else:
                # open all files already processed (these will contain '.nc' extension)
            ds_swt = xr.open_mfdataset([s + '.nc' for s in files[:j][-3:]], concat_dim='Time', 
                             combine='nested', compat='override', coords='all', 
                                drop_variables= ['UH_SWT02','UH_SWT25','UH_SWT25_WRF',
                                                'WZ_SWT02', 'WZ_SWT25', 'CREF_SWT'])
            ...
            ds_swt = xr.concat([ds[swt_vars], ds_swt[swt_vars]], dim='Time')
                # calculate swaths
            uh_15swt_02 = np.expand_dims(np.amax(ds_swt['UH_02'].values, 0), 0)
            uh_15swt_25 = np.expand_dims(np.amax(ds_swt['UH_25'].values, 0), 0)
            uh_15swt_wrf25 = np.expand_dims(np.amax(ds_swt['UP_HELI_MAX'].values, 0), 0)
            wz_15swt_02 = np.expand_dims(np.amax(ds_swt['WZ_02'].values, 0), 0)
            wz_15swt_25 = np.expand_dims(np.amax(ds_swt['WZ_25'].values, 0), 0)
            dbz_15swt = np.expand_dims(np.amax(ds_swt['REFD_MAX'].values, 0), 0)
        ...
        ds['CREF_SWT'].attrs['description'] = 'Maximum composite reflectivity swaths'
            ...
            # hourly swaths
        ds['UH_HRSWT02'] = (['Time', 'south_north', 'west_east'], uh_hrswt_02)
        ds['UH_HRSWT02'].attrs['units'] = 'm2/s2'
        ds['UH_HRSWT02'].attrs['description'] = 'Hourly 0-2 km UH swaths'

        ds['UH_HRSWT25'] = (['Time', 'south_north', 'west_east'], uh_hrswt_25)
        ds['UH_HRSWT25'].attrs['units'] = 'm2/s2'
        ds['UH_HRSWT25'].attrs['description'] = 'Hourly 2-5 km UH swaths'

        ds['UH_HRSWT25_WRF'] = (['Time', 'south_north', 'west_east'], uh_hrswt_wrf25)
        ds['UH_HRSWT25_WRF'].attrs['units'] = 'm2/s2'
        ds['UH_HRSWT25_WRF'].attrs['description'] = 'Hourly 2-5 km UH swaths from raw WRF output'

        ds['WZ_HRSWT02'] = (['Time', 'south_north', 'west_east'], wz_hrswt_02)
        ds['WZ_HRSWT02'].attrs['units'] = 'm2/s2'
        ds['WZ_HRSWT02'].attrs['description'] = 'Hourly 0-2 km vertical vorticity swaths'

        ds['WZ_HRSWT25'] = (['Time', 'south_north', 'west_east'], wz_hrswt_25)
        ds['WZ_HRSWT25'].attrs['units'] = 'm2/s2'
        ds['WZ_HRSWT25'].attrs['description'] = '2-5 km vertical vorticity swaths'

        ds['CREF_HRSWT'] = (['Time', 'south_north', 'west_east'], dbz_hrswt)
        ds['CREF_HRSWT'].attrs['units'] = 'm2/s2'
        ds['CREF_HRSWT'].attrs['description'] = 'Hourly maximum composite reflectivity swaths'

            # Write 30-minute swaths
        ds['UH_30SWT02'] = (['Time', 'south_north', 'west_east'], uh_30swt_02)
        ds['UH_30SWT02'].attrs['units'] = 'm2/s2'
        ds['UH_30SWT02'].attrs['description'] = '30-min 0-2 km UH swaths'

        ds['UH_30SWT25'] = (['Time', 'south_north', 'west_east'], uh_30swt_25)
        ds['UH_30SWT25'].attrs['units'] = 'm2/s2'
        ds['UH_30SWT25'].attrs['description'] = '30-min 2-5 km UH swaths'

        ds['UH_30SWT25_WRF'] = (['Time', 'south_north', 'west_east'], uh_30swt_wrf25)
        ds['UH_30SWT25_WRF'].attrs['units'] = 'm2/s2'
        ds['UH_30SWT25_WRF'].attrs['description'] = '30-min 2-5 km UH swaths from raw WRF UH output'

        ds['WZ_30SWT02'] = (['Time', 'south_north', 'west_east'], wz_30swt_02)
        ds['WZ_30SWT02'].attrs['units'] = 'm2/s2'
        ds['WZ_30SWT02'].attrs['description'] = '30-min 0-2 km vertical vorticity swaths'

        ds['WZ_30SWT25'] = (['Time', 'south_north', 'west_east'], wz_30swt_25)
        ds['WZ_30SWT25'].attrs['units'] = 'm2/s2'
        ds['WZ_30SWT25'].attrs['description'] = '30-min 2-5 km vertical vorticity swaths'

        ds['CREF_30SWT'] = (['Time', 'south_north', 'west_east'], dbz_30swt)
        ds['CREF_30SWT'].attrs['units'] = 'm2/s2'
        ds['CREF_30SWT'].attrs['description'] = '30-min maximum composite reflectivity swaths'

            # Write 15-min swaths
        ds['UH_15SWT02'] = (['Time', 'south_north', 'west_east'], uh_15swt_02)
        ds['UH_15SWT02'].attrs['units'] = 'm2/s2'
        ds['UH_15SWT02'].attrs['description'] = '15-min 0-2 km UH swaths'

        ds['UH_15SWT25'] = (['Time', 'south_north', 'west_east'], uh_15swt_25)
        ds['UH_15SWT25'].attrs['units'] = 'm2/s2'
        ds['UH_15SWT25'].attrs['description'] = '15-min 2-5 km UH swaths'

        ds['UH_15SWT25_WRF'] = (['Time', 'south_north', 'west_east'], uh_15swt_wrf25)
        ds['UH_15SWT25_WRF'].attrs['units'] = 'm2/s2'
        ds['UH_15SWT25_WRF'].attrs['description'] = '15-min 2-5 km UH swaths from raw WRF UH output'

        ds['WZ_15SWT02'] = (['Time', 'south_north', 'west_east'], wz_15swt_02)
        ds['WZ_15SWT02'].attrs['units'] = 'm2/s2'
        ds['WZ_15SWT02'].attrs['description'] = '15-min 0-2 km vertical vorticity swaths'

        ds['WZ_15SWT25'] = (['Time', 'south_north', 'west_east'], wz_15swt_25)
        ds['WZ_15SWT25'].attrs['units'] = 'm2/s2'
        ds['WZ_15SWT25'].attrs['description'] = '15-min 2-5 km vertical vorticity swaths'

        ds['CREF_15SWT'] = (['Time', 'south_north', 'west_east'], dbz_15swt)
        ds['CREF_15SWT'].attrs['units'] = 'm2/s2'
        ds['CREF_15SWT'].attrs['description'] = '15-min maximum composite reflectivity swaths'

        # Write sounding profiles
        ds['PSND'] = (['Time', 'bottom_top', 'south_north_snd', 'west_east_snd'], np.expand_dims(p_snd, 0))
        ds['PSND'].attrs['units'] = 'hectopascal'
        ds['PSND'].attrs['description'] = 'Vertical pressure profiles every 10 gridpoints'

        ds['TSND'] = (['Time', 'bottom_top', 'south_north_snd', 'west_east_snd'], np.expand_dims(t_snd, 0))
        ds['TSND'].attrs['units'] = 'degree Celsius'
        ds['TSND'].attrs['description'] = 'Vertical temperature profiles every 10 gridpoints'

        ds['TDSND'] = (['Time', 'bottom_top', 'south_north_snd', 'west_east_snd'], np.expand_dims(td_snd, 0))
        ds['TDSND'].attrs['units'] = 'degree Celsius'
        ds['TDSND'].attrs['description'] = 'Vertical dewpoint profiles every 10 gridpoints'

        ds['USND'] = (['Time', 'bottom_top', 'south_north_snd', 'west_east_snd'], np.expand_dims(u_snd, 0))
        ds['USND'].attrs['units'] = 'knot'
        ds['USND'].attrs['description'] = 'Vertical u-wind profiles every 10 gridpoints'

        ds['VSND'] = (['Time', 'bottom_top', 'south_north_snd', 'west_east_snd'], np.expand_dims(v_snd, 0))
        ds['VSND'].attrs['units'] = 'hectopascal'
        ds['VSND'].attrs['description'] = 'Vertical v-wind profiles every 10 gridpoints'

        ds['ZSND'] = (['Time', 'bottom_top', 'south_north_snd', 'west_east_snd'], np.expand_dims(z_snd, 0))
        ds['ZSND'].attrs['units'] = 'meter'
        ds['ZSND'].attrs['description'] = 'Vertical height profiles every 10 gridpoints'

        # Add south_north_snd and west_east_snd as sounding coordinates
        ds['XLAT_SND'] = (['Time', 'south_north_snd', 'west_east_snd'], np.expand_dims(lat_snd, 0))
        ds['XLAT_SND'].attrs['units'] = 'degree_north'
        ds['XLAT_SND'].attrs['description'] = 'Latitude for sounding variables'

        ds['XLONG_SND'] = (['Time', 'south_north_snd', 'west_east_snd'], np.expand_dims(lon_snd, 0))
        ds['XLONG_SND'].attrs['units'] = 'degree_east'
        ds['XLONG_SND'].attrs['description'] = 'Longitude for sounding variables'

        ds = ds.set_coords(['XLAT_SND', 'XLONG_SND'])
